 ...
 * state in one place (backup = copy papert-claw.db).
 */
	...
	const keys: AuthenticationState["keys"] = makeCacheableSignalKeyStore(
		{
			...
			async set(data: Record<string, Record<string, unknown>>) {
				for (const [type, entries] of Object.entries(data) as Array<[keyof SignalDataTypeMap & string, Record<string, unknown>]>) {
