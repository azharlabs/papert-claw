...
export class SlackBot {
	...
	async start(): Promise<void> {
		...
		this.app.use(async (args) => {
			const body = args.body as { type?: string; event?: { type?: string; channel?: string; user?: string } };
			if (body.event?.type) {
				this.logger.debug(
					{
						bodyType: body.type,
		...
		this.app.message(async ({ message }) => {
			if ("bot_id" in message && message.bot_id) {
				this.logger.debug({ botId: message.bot_id }, "Ignoring bot-authored message event");
				return;
			}
			if (!("channel_type" in message) || message.channel_type !== "im") {
				this.logger.debug(
					{ channelType: "channel_type" in message ? message.channel_type : undefined },
					"Ignoring non-DM message event",
				);
				return;
			}
			if (!("user" in message) || !message.user) {
				this.logger.debug("Ignoring DM event without user");
				return;
			}
			...
			if (!hasText && !hasFiles) {
				this.logger.debug("Ignoring DM event without text/files");
				return;
			}

				const files: SlackFile[] = rawFiles.map((f) => ({
			...
			if (this.handler) {
				this.logger.info(
					{ channelId: message.channel, userId: message.user, hasText: Boolean(hasText), fileCount: files.length },
					"Dispatching DM to app handler",
				);
				await this.handler({
					type: "dm",
		...
		this.app.event("app_mention", async ({ event }) => {
			if (!this.mentionHandler) {
				this.logger.debug("Ignoring app_mention: mention handler not registered");
				return;
			}
			if (!event.user) {
				this.logger.debug("Ignoring app_mention without user");
				return;
			}
			...
			if (!cleanText && !hasFiles) {
				this.logger.debug("Ignoring app_mention without text/files after mention stripping");
				return;
			}
			...
			this.logger.info(
				{ channelId: event.channel, userId: event.user, hasText: Boolean(cleanText), fileCount: files.length },
				"Dispatching app mention to app handler",
			);
