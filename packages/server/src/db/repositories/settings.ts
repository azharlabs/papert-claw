...
export function createSettingsRepository(db: Kysely<DB>) {
	return {
		...
		async update(
			...
			if (data.openaiApiKey !== undefined) updates.openai_api_key = data.openaiApiKey;
			if (data.openaiBaseUrl !== undefined) updates.openai_base_url = data.openaiBaseUrl;
			if (data.openaiModel !== undefined) updates.openai_model = data.openaiModel;
