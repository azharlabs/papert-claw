...
describe("WhatsApp endpoints", () => {
	...
	describe("GET /api/whatsapp/pair", () => {
		...
		it("returns 500 when pairing startup fails", async () => {
			await seedAdmin(db);
			const whatsapp = makeMockWhatsApp({
				startPairing: async () => {
					throw new Error("pairing init failed");
				},
			} as Partial<WhatsAppBot>);
			const app = createApp(db, config, { whatsapp });
			const cookie = await loginAdmin(app);
			...
			expect(res.status).toBe(500);
			const body = await res.json();
			expect(body.error.code).toBe("PAIRING_FAILED");
		});
	});
...
describe("Auth endpoints", () => {
	...
	describe("POST /api/auth/login", () => {
		...
		it("returns 200 with valid credentials and sets cookie", async () => {
			...
			expect(res.headers.get("set-cookie")).toContain("papert-claw_session=");
		});
...
describe("Auth middleware", () => {
	...
	it("blocks protected routes during setup mode", async () => {
		const whatsapp = { isConnected: true } as unknown as WhatsAppBot;
		const app = createApp(db, config, { whatsapp });
		const res = await app.request("/api/whatsapp/status");
		expect(res.status).toBe(503);
	...
	it("allows setup routes during setup mode", async () => {
		const app = createApp(db, config);
