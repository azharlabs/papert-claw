...
describe("OnboardingPage navigation and flow", () => {
	...
	it("persists the latest identity values after navigating back to Identity", async () => {
		...
		await user.type(screen.getByLabelText("Bot Name"), "Papert Claw");
		await user.click(screen.getByRole("button", { name: "Continue" }));
		await waitFor(() => {
		...
		await user.clear(screen.getByLabelText("Bot Name"));
		await user.type(screen.getByLabelText("Bot Name"), "Papert Claw Pro");
		await user.click(screen.getByRole("button", { name: "Continue" }));
		await waitFor(() => {
			expect(screen.getByText("Connect your channels")).toBeInTheDocument();
		});
		...
		expect(identityBodies).toEqual([
			{ orgName: "Acme", botName: "Papert Claw" },
			{ orgName: "Acme Labs", botName: "Papert Claw Pro" },
		]);
	});
	...
	it("uses updated identity values in later onboarding steps", async () => {
		...
		await user.type(screen.getByLabelText("OPENAI API Key"), "sk-api-test-key");
		await user.click(screen.getByRole("button", { name: "Connect" }));
		await waitFor(() => {
		...
		expect(screen.getByText("Papert Claw is ready")).toBeInTheDocument();
	});
